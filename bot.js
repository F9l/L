...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@177417683774603264>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "السلامييي عليكم") {
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setVc")) {
    ...
    channelfind.setName(
      `DarkMoon Online [${message.guild.members.filter(m => m.voiceChannel).size}]` ///تعديل غير اساسي تعديل اسم روم الفويس اونلاين
    );
